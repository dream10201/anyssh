...
服务端同时内嵌以下 Linux 客户端，安装脚本会根据 `uname -m` 自动选择：

- `amd64`：x86-64。
- `arm64`：AArch64。
- `arm`：32 位 ARM，使用 `GOARM=5` 构建，兼容 ARMv5 及更新版本。
...
GitHub Action 会发布 `linux/amd64`、`linux/arm64` 多平台镜像到 GHCR。镜像中的服务端均携带 `amd64`、`arm64`、32 位 `arm` 三种客户端。
