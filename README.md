...
打开 `http://服务端地址/admin/` 可以查看当前连接客户端、设备信息及各自链接，并执行禁用、设置链接过期时间和立即更换链接。页面顶部可修改全局链接刷新分钟数，`0` 表示永久不自动刷新；设置会立即下发给在线客户端。客户端重连时会回传它收到的刷新值和版本，服务端重启后采用版本最新的客户端配置；若没有客户端携带修改版本，则显示环境变量默认值。每个客户端都会生成独立链接。

刷新设置只保存在进程内存中。如果服务端与所有收到修改的客户端进程同时重启，将恢复 `ANYSSH_CLIENT_ROTATE`；该设计不需要任何落地状态文件。
